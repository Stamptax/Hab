...
export const fetchTweets = unstable_cache(
  async () => {
    try {
  ...
  async () => {
    try {
      ...
      if (!detailsResponse.ok) {
        ...
        console.log(
          `✅ YouTube videos fetched: ${processedVideos.length} items`
        );
      ...
      const nonLiveVideos =
        detailsData.items?.filter(
          (video: any) => !video.liveStreamingDetails
        ) || [];
